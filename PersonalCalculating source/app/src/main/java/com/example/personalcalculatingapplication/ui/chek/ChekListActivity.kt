...
import com.example.personalcalculatingapplication.adapters.ChekItemsAdapter
...
class ChekListActivity : AppCompatActivity(), ChekItemsAdapter.AdapterListener {

    lateinit var item: CalculatingDatabaseHelper
    var selectedFirstDate = ""
    var selectedSecondDate = ""
    lateinit var mList: ArrayList<Chek>
    lateinit var adapter: ChekItemsAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mList = item.getAllCheks() as ArrayList<Chek>
        rcv_items.layoutManager = LinearLayoutManager(this)
        adapter = ChekItemsAdapter(this)
    ...
    private fun getDiffList() {
        val finalList = ArrayList<Chek>()
        if (selectedFirstDate.isNotEmpty()) {
            if (selectedSecondDate.isNotEmpty()) {
                // has both date
                for (i in mList) {
                    val cDate = i.createdDate.replace("/", "-") + " 00:00:00"
