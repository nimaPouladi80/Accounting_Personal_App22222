...
import com.example.personalcalculatingapplication.adapters.IncomeItemsAdapter
...
class IncomeListActivity: AppCompatActivity() , IncomeItemsAdapter.AdapterListener{

    lateinit var item: CalculatingDatabaseHelper
    var selectedFirstDate = ""
    var selectedSecondDate = ""
    lateinit var mList: ArrayList<Income>
    lateinit var adapter: IncomeItemsAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mList= item.getAllIncome() as ArrayList<Income>
      rcv_items.layoutManager = LinearLayoutManager(this)
        adapter = IncomeItemsAdapter(this)
    ...
    private fun getDiffList() {
        val finalList = ArrayList<Income>()
        if (selectedFirstDate.isNotEmpty()) {
            if (selectedSecondDate.isNotEmpty()) {
                // has both date
                for (i in mList) {
